...
Runs DER++ with exact hyperparameters from Buzzega et al. (2020) paper.
No extra features, no wrappers, just pure DER++.

This establishes the baseline that Causal-DER must beat.
"""
...
def create_split_cifar100(num_tasks=5, data_root='../../data'):
    ...
    Standard split: 5 tasks, 20 classes per task.
    """
...
def run_der_plus_plus_benchmark(seed=42, data_root='../../data'):
    """
    Run DER++ benchmark with exact paper settings.
    
    Hyperparameters from Buzzega et al. (2020):
    - Learning rate: 0.03
    ...
    for task_id in range(5):
        ...
        # Show buffer stats
        stats = der_engine.get_statistics()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Run clean DER++ benchmark')
    ...
    for run in range(args.num_runs):
        ...
        print(f"{'#'*60}\n")
        results = run_der_plus_plus_benchmark(seed=seed, data_root=args.data_root)
